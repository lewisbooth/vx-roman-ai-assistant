...
On development stores, **Development ? Developer tools ? Voice** includes all 22 built-in GPT-Live voices and defaults to **Willow** (lively Irish English). Its selection is saved for this tab; switch to text before choosing another voice and reconnecting. Startup sends one authenticated readiness signal after WebRTC connects and attaches audio, independently of the playback promise. Expand `[Roman] Voice startup timing.` in the browser's debug console to compare connection stages; these timings exclude transcript content and do not measure audible speech quality.
