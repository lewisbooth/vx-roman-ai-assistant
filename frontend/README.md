...
The standalone preview displays the interface only. Verify real chat on an installed development store after approving `write_app_proxy` in Shopify admin. Luna can search, look up products and navigate on request through the same storefront tools used by the drawer. `src/session/storefront-executor.ts` serializes catalog reads and navigation with a bounded queue. Execution is claimed once; lost result acknowledgements retry the saved outcome without repeating the action. Navigation acknowledges only a confirmed page change. Full-page fallback can interrupt confirmation and is never replayed automatically. Model cart/measurement actions are not connected yet.
