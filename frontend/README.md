...
The theme registry selects hooks by permanent Shopify shop identity. Development stores reuse production theme hooks where behavior matches. Navigation accepts any same-storefront path; there are no configured route lists. Unknown stores remain unconfigured. Local demo pages belong to `vite.config.ts`, and representative navigation test routes belong to test fixtures.
