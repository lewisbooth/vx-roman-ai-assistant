...
The standalone preview displays the interface only. Verify real chat on an installed development store after approving `write_app_proxy` in Shopify admin. Terra can search, look up products and navigate through the same storefront tools used by the drawer, including proactively opening a clearly chosen product. The [server prompt](../admin/prompts/roman.server.ts) owns that policy. `src/session/storefront-executor.ts` serializes catalog reads and navigation with a bounded queue. Execution is claimed once; lost result acknowledgements retry the saved outcome without repeating the action. Navigation acknowledges only a confirmed destination. Full-page fallback can interrupt confirmation and is never replayed automatically. Cart reads and persistent measurement tools are connected in text and voice. Cart changes show a specific approval panel before execution; it remains accessible when the sidebar is closed. Measurement application follows a single width/drop/unit confirmation in the conversation, without another panel. Both paths retain one-use claims and check current state immediately before acting.
