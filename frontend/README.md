...
Live Shopify calls require the installed storefront and its browser session; localhost reports that limitation instead of returning sample data. Tool runs are explicit, single-flight and bounded; removing Roman cancels its pending observations/requests and clears drafts. Cancellation cannot undo a cart change already handed to the theme. Tool results exclude cart tokens, notes and customer attributes; they stay in the drawer and are not logged or persisted.
