...
Measurement drafts persist through reloads and backend restarts with their conversation. `kind` distinguishes measured `window` dimensions (the manual default) from confirmed `order` dimensions; `mount` is `recess`, `exact` or `unknown`. Saving preserves the exact values and units without conversion or deductions. The model's `apply_measurements({productPath})` proposes filling the matching PDP only: it requires an order draft, explicit approval, the same selected mm/cm unit and valid active theme fields. It never changes fitting, submits the form or promises a configured price. Unsupported controls stay manual.
