...
[`prompts/roman.server.ts`](prompts/roman.server.ts) is the single home for Roman's shared character, text advisor, voice style/opening and delegated voice briefing. `ROMAN_PREAMBLE` owns the full welcome used for a new voice conversation or text greeting; a specific initial text request gets a short introduction and a relevant answer. Resumed conversations pick up the customer's topic. Text uses concise Markdown and verified product links. The frontend renders it safely; the database retains the original text. Prompt changes require rebuilding the backend container and restarting any active voice connection.

| Module                                           | Owns                                                                       |
| ------------------------------------------------ | -------------------------------------------------------------------------- |
| `prompts/roman.server.ts`                        | Roman's character, text/voice instructions and current capability boundaries |
...
Cart/measurement model tools and photo uploads/visualization remain subsequent phases. Catalog matches do not establish fitting suitability, stock or a configured quote.
...
Roman explicitly selects Live's `ballad` voice for the British delivery checkpoint. The shared voice style requests warm, calm southern British pronunciation and flowing, unhurried speech, including after delegated work. Voice selection and prompts do not guarantee an accent or verbatim playback: validate the opening and follow-up speech by listening. To change the base voice, set `session.audio.output.voice` at session creation in the provider and start a new connection. Tone, pace, pronunciation and opening copy belong in the [prompt file](prompts/roman.server.ts), not the transport.
...
`voice/repository.server.ts` owns `VoiceSession` leases and exact `VoiceTranscript` fragments. The conversation repository groups captions for display and merges them with text, page visits and widgets using one server sequence. Same-speaker captions tolerate pauses up to three seconds and skip empty delegation bookkeeping; visible entries and speaker/connection changes still separate bubbles. Grouping preserves exact caption text and the first fragment's ID. Captions are transcription observations, not proof the customer heard every word. Initial GPT-Live context is a bounded recent extract; the full persisted conversation remains available to Luna and later text turns. Raw audio is never stored by Roman.
...
`usage/` owns provider usage contracts and persistence. Each Responses request, including tool-loop rounds and voice delegation, gets one durable `ModelUsage` attempt before the API call. Terminal provider counts replace that attempt once, independently of whether the conversation was cancelled or ended. Input/output/total tokens and available cached-input/reasoning subsets come from the provider. Final GPT-Live cumulative seconds are saved on `VoiceSession`; intermediate updates are not added together. The usage migration preserves existing sessions and leaves their unknown counts null.

Totals cover recorded usage only. Missing reports, unfinished calls and older sessions are shown explicitly; zero means a reported zero. A process crash can leave an attempt pending without final counts. This is conversation-level operational usage, not a billing reconciliation or a currency estimate. Rebuild/recreate Docker to deploy this console and its migrations; these backend-only changes do not require a Shopify extension release.
