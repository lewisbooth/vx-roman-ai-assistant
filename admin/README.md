...
The server-local `ask_question` tool adds one follow-up beneath the reply's other widgets, with one to four short answers (usually two or three). It persists the question and original options with the successful reply. Choosing an answer submits ordinary customer text; during an active voice connection it continues GPT-Live without stopping audio or starting a separate Terra text reply. Typing or speaking also retires the choices, while the question remains in the transcript. Admin inspection retains the original options as read-only history. Recommendations use a brief overview and product cards instead of a duplicate product list; the question tool supports the next useful decision without replacing cart or measurement confirmation rules. After a successful question tool call, text keeps the widget as the sole written question. Voice gives its overview, then says that exact displayed question once; it does not add or rephrase a second question, and reads options only when useful.
...
Authenticated `POST /api/conversations/:id/voice/:voiceId/answers` accepts `{clientId, requestId, questionId, answer}` only from the active voice owner and only for the current saved question's offered answer. It atomically saves an ordinary user text row with `id=requestId` and `questionAnswer: {questionId, voiceId}` provenance; it never creates a voice caption. The trusted sideband supplies the selection through acknowledged `thinking.append` context and one `commentary.append` continuation cue, following [Live's context-update contract](https://developers.openai.com/api/docs/guides/live-conversations#add-context-during-the-conversation). Live can delegate subsequent work through the existing Terra runner. Receipts deduplicate across retry/restart, including after voice ends, and never replay provider delivery. If delivery is unconfirmed, the endpoint returns an actionable 503 while retaining the selected answer; the browser rereads its durable receipt. A crash between persistence and delivery leaves the same saved answer without an automatic resend. There are at most 40 selected answers per conversation. Provider acknowledgment confirms context acceptance, not spoken words or completed playback.
