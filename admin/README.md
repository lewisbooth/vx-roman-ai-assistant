...
Cart and measurement tools share the text runner used by voice delegation. `get_cart` reads the current storefront basket. `add_to_cart`, `remove_from_cart`, `set_cart_quantity`, `clear_cart` and `apply_measurements` require explicit browser approval before their one-use claim. Confirmation is transport state, never a model argument. At most one mutation is proposed per reply. Sanitized outcomes persist for later turns; an interrupted or unconfirmed submission must be checked rather than replayed. Catalog matches do not establish fitting suitability, stock or a configured quote. Photo uploads and visualization remain later work.

`measurements/` owns product-scoped drafts and idempotent writes. Authenticated `POST /api/conversations/:id/measurements` accepts `{requestId, name, arguments}` for manual get/set; model calls use the same service. Drafts preserve width, drop, units, window/order meaning and mounting without conversion or deductions. Limits are 20 products and 200 manual request receipts per conversation. An old request returns its original result without overwriting newer values. Ended conversations reject writes. Applying dimensions additionally freezes and revalidates the saved draft at claim time.
...
`voice/repository.server.ts` owns `VoiceSession` leases and exact `VoiceTranscript` fragments. The conversation repository groups captions for display using server sequence; same-speaker captions tolerate pauses up to three seconds. New voice product widgets record their connection and result-completion sequence in `partsJson`. They appear as soon as ready, then follow that response's captions, stopping at another message or speaker/connection change. The same projection serves the sidebar and admin without rewriting captions or inventing spoken text. Initial GPT-Live context is a bounded recent extract; the full persisted conversation remains available to Luna. Roman never stores raw audio, and captions do not prove the customer heard every word.
