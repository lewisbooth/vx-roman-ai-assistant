...
[`prompts/roman.server.ts`](prompts/roman.server.ts) is the single home for Roman's shared character, text advisor, voice style/opening and delegated voice briefing. New conversations introduce Roman as a digital shop-at-home advisor; resumed conversations pick up the customer's topic. Text uses concise Markdown and verified product links. The frontend renders it safely; the database retains the original text. Prompt changes require rebuilding the backend container and restarting any active voice connection.
...
Roman uses Live's default `marin` voice. `ROMAN_VOICE_PROMPT` requests warm, calm British English with an unhurried pace; regional accent consistency needs listening checks. To change the base voice, set `session.audio.output.voice` at session creation in the provider and start a new connection. Tone, pace, pronunciation and opening copy belong in the [prompt file](prompts/roman.server.ts), not the transport.
