...
export const navigationToolDefinition = {
  ...
  name: "navigate",
  description:
    "Synchronize a verified public storefront page behind Roman's fullscreen interface. Open the verified PDP for the customer's explicit first product choice, or a replacement after Roman receives conversational replacement confirmation through ask_question, including the compatible-transfer choice when work has begun. Keep the current active blind until replacement is confirmed. Recommendations and single search results do not select a blind. Use search and show_products for collection browsing instead of a collection page. This tool changes only the hidden storefront, not Roman's visible view. Use show_view for an explicit request to see Cart, Gallery or Chat; never open Cart automatically after adding a product or sample. Respect requests to keep comparing or decline product navigation. Use a catalog/conversation path. This tool cannot visit account, checkout, app/API, cart-action or cart-permalink URLs. Model navigation rejects redirects and unsafe theme swaps without automatically reloading. On failure, explain the limitation and offer returning to the storefront by closing Roman; do not promise a transcript link or retry automatically. A full page load interrupts voice.",
  strict: true,
  parameters: {
    type: "object",
